...
/datum/error_viewer/Topic(href, href_list)
	. = ..()
	//var/mob/user = usr
	//	if(user.check_rights(R_ADMIN)), usr is apparently unsafe but why stop people from looking at runtimes if they want to href exploit
	if(href_list["viewruntime"])
		var/datum/error_viewer/error_viewer = locate(href_list["viewruntime"])

		if(href_list["viewruntime_backto"])
			error_viewer.show_to(usr, locate(href_list["viewruntime_backto"]), href_list["viewruntime_linear"])
		else
