...
/world/Error(var/exception/e)
	...
	cooldown = max(0, cooldown - (world.time - last_seen)) + CONFIG_ERROR_COOLDOWN
	// ... which is used to silence an error if it occurs too often, too fast
	if (cooldown > CONFIG_ERROR_COOLDOWN * CONFIG_ERROR_LIMIT)
		cooldown = -1
		silencing = 1
		spawn (0)
			usr = null
			sleep(CONFIG_ERROR_SILENCE_TIME)
	...
	if (splitlines.len > CONFIG_ERROR_USEFUL_LEN) // If there aren't at least three lines, there's no info
	...
	if (silencing)
		desclines += "  (This error will now be silenced for [CONFIG_ERROR_SILENCE_TIME / 600] minutes)"
