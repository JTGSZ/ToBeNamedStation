...
// Main loop.
/*
	JTGSZ EDIT - 9/16/2023
	To be fair we just go straight into the world after a short init.
	With SS13, we have Init - Lobby delay - Round start - Game goin - Game shutdown - Repat
	With This, we have init - Game Goin - Game Shutdown - Repeat

	Thus for posterities sake, I've just commented out the lobby subsystems and roundstarted state, as we don't got those delays.
*/
/datum/controller/master/proc/Loop()
	...
	//JTGSZ - The Tickers jus be added to both prev
	...
	while (1)
		...
		if (!queue_head || !(iteration % 3))
			// JTGSZ - We only got one ss list now.
			//if (round_started)
